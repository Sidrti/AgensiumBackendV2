...
**Purpose:** Step-by-step implementation guide for V2.1 migration with simplified model
...
| Phase 2: Task API            | 2-3 days        | Phase 1      |
| Phase 3: Transformer Updates | 2-3 days        | Phase 2      |
| Phase 4: Downloads & Results | 1-2 days        | Phase 3      |
| Phase 5: Testing & Migration | 2-3 days        | Phase 4      |
| **Total**                    | **~10-14 days** |              |

**Faster than V2** due to simplified model!
...
### 1.2 Add Simplified Task Model

**Update:** `db/models.py`

Add the simplified Task model as defined in [05_DATABASE_SCHEMA_V2.md](05_DATABASE_SCHEMA_V2.md).

**Key Points:**

- Only 18 fields (vs 30+ in V2)
- No `parameters` field
- No `files_metadata` field
- No result storage fields
...
```bash
alembic revision --autogenerate -m "Add simplified tasks table V2.1"
...
```sql
-- See 05_DATABASE_SCHEMA_V2.md for complete SQL
CREATE TABLE tasks (
    ...
    -- ... (18 fields total, see schema doc)
);
```

### 1.4 Add Simplified Task Schemas
...
```python
"""
Task API Routes for V2.1 Architecture (Simplified)
"""
import uuid
...
async def create_task(
    ...
    """Create a new analysis task. Parameters are NOT included here."""
...
async def get_upload_urls(
    ...
    """Generate presigned URLs for files AND parameters.json."""
    # Get task
    ...
    return schemas.UploadUrlsResponse(
        ...
        message="Upload files and parameters directly to the provided URLs using PUT method."
    )
...
async def trigger_processing(
    ...
    # For V2.1, we derive expected files from tool definition
    from main import TOOL_DEFINITIONS
    ...
    for file_key, file_def in tool_files.items():
        if file_def.get("required", False):
            # Check if file exists (we don't know exact filename, so list prefix)
            prefix = f"users/{current_user.id}/tasks/{task_id}/inputs/"
            files = s3_service.list_files(prefix)

            # Simple check: at least one file should exist
            if not files:
    ...
    # For V2.1 synchronous: Execute immediately
    try:
...
async def get_downloads(
    ...
    for file_info in output_files:
        ...
        url = s3_service.generate_download_url(file_info['key'])
...
# ... (list, cancel, delete endpoints same as V2)
```
...
### 3.1 Create V2.1 Transformer Functions

Add new functions to each transformer that read from S3 including parameters.
...
async def run_profile_my_data_analysis_v2_1(
    ...
    V2.1 version - reads files AND parameters from S3.
    """
    from services.s3_service import s3_service
    ...
    try:
        ...
        input_files = s3_service.list_files(task.get_input_prefix())

        files_map = {}
        for file_info in input_files:
            filename = file_info['filename']

            # Skip parameters.json
            if filename == 'parameters.json':
                continue

            # Determine file key (primary, baseline, etc.)
            file_key = _determine_file_key(filename)
        ...
        with BillingContext(current_user) as billing:
            for agent_id in task.agents:
                try:
                    ...
                    task.progress = int((len(agent_results) / len(task.agents)) * 80) + 15
                    db.commit()
...
def _determine_file_key(filename: str) -> str:
...
async def _upload_outputs_to_s3_v2_1(
    ...
    for download in downloads:
        ...
        # Build S3 key using task helper method
...
Outputs are uploaded to S3, downloads endpoint returns presigned URLs (already implemented in Phase 2).

**No changes needed** - downloads module logic remains the same, just outputs go to S3.
...
```
1. Unit Tests
   - [ ] S3Service - all methods including parameters
   - [ ] Task creation (simplified model)
   - [ ] Upload URL generation (files + parameters)
   - [ ] File & parameter verification
   - [ ] Task status transitions

2. Integration Tests
   - [ ] Full flow: create → upload files → upload params → process → download
...
- [ ] Simplified schema reviewed

Migration:
- [ ] Run database migration
- [ ] Deploy new code
- [ ] Verify task endpoints work
- [ ] Test full flow with parameters in S3
...
| File                                          | Changes                                 |
| --------------------------------------------- | --------------------------------------- |
| `db/models.py`                                | Add simplified Task model (18 fields)   |
| `db/schemas.py`                               | Add simplified task schemas             |
| `main.py`                                     | Register task routes                    |
| `transformers/profile_my_data_transformer.py` | Add V2.1 function (read params from S3) |
| `transformers/clean_my_data_transformer.py`   | Add V2.1 function (read params from S3) |
| `transformers/master_my_data_transformer.py`  | Add V2.1 function (read params from S3) |

### Database Changes

| Table   | Change                            |
| ------- | --------------------------------- |
| `tasks` | NEW table (simplified, 18 fields) |
| `users` | Add relationship to tasks         |

---

## Benefits of V2.1 Implementation

### Simplifications Achieved

✅ **40% fewer database fields**  
✅ **95% reduction in database storage**  
✅ **No parameters in database**  
✅ **No file metadata duplication**  
✅ **No result storage in database**  
✅ **Cleaner data model**  
✅ **Faster implementation** (~10-14 days vs ~2-3 weeks)

### For Users

- ✅ Same great UX as V2
- ✅ Parameters handled transparently
- ✅ Faster response times (less DB overhead)

### For System

- ✅ Lower database load
- ✅ Simpler queries
- ✅ S3 is single source of truth
- ✅ Easier to maintain
