# Database Schema V2 - Task Model & Changes

**Document Version:** 1.0  
**Created:** December 19, 2025  
**Purpose:** Database schema changes for V2 task-based architecture
...
| ------------- | -------------------- | ------------------------ |
| **NEW**       | `Task`               | Core task tracking model |
...
│     agents_completed JSON           ["unified-profiler", "drift-detector"]  │
│                                                                             │
│     -- File Information --                                                  │
│     files_metadata   JSON           {primary: {filename, size, type}}       │
│     s3_input_keys    JSON           ["users/1/tasks/abc/inputs/file.csv"]   │
│     s3_output_keys   JSON           ["users/1/tasks/abc/outputs/report.xlsx"]│
│                                                                             │
│     -- Results --                                                           │
│     result_summary   JSON           {alerts: 5, issues: 10, score: 85}      │
│     result_full      JSON           Full agent results (optional storage)   │
│                                                                             │
│     -- Error Information --                                                 │
│     error_code       VARCHAR(50)    BILLING_INSUFFICIENT_CREDITS, etc       │
│     error_message    TEXT           Human-readable error                    │
│     failed_agent     VARCHAR(100)   Agent that failed                       │
│     partial_results  JSON           Results from completed agents           │
│                                                                             │
│     -- Timestamps --                                                        │
│     created_at       DATETIME       Task creation time                      │
│     upload_started_at DATETIME      When upload URLs generated              │
│     files_verified_at DATETIME      When files confirmed in S3              │
...
│     expiry_reason    VARCHAR(100)   Why task expired                        │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
CREATE TABLE tasks (
    ...
    -- File Information
    files_metadata JSON NOT NULL,
    s3_input_keys JSON,
    s3_output_keys JSON,

    -- Results
    result_summary JSON,
    result_full LONGTEXT,  -- Can be large, use LONGTEXT for MySQL
...
```python
"""Add tasks table for V2 architecture

Revision ID: v2_001_add_tasks
...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('agents_completed', mysql.JSON, nullable=True),

        # File Information
        sa.Column('files_metadata', mysql.JSON, nullable=False),
        sa.Column('s3_input_keys', mysql.JSON, nullable=True),
        sa.Column('s3_output_keys', mysql.JSON, nullable=True),

        # Results
        sa.Column('result_summary', mysql.JSON, nullable=True),
        sa.Column('result_full', mysql.LONGTEXT, nullable=True),

        # Error Information
        sa.Column('error_code', sa.String(50), nullable=True),
        sa.Column('error_message', sa.Text, nullable=True),
        sa.Column('failed_agent', sa.String(100), nullable=True),
        sa.Column('partial_results', mysql.JSON, nullable=True),

        # Timestamps
        sa.Column('created_at', sa.DateTime, server_default=sa.func.now(), nullable=False),
        sa.Column('upload_started_at', sa.DateTime, nullable=True),
        sa.Column('files_verified_at', sa.DateTime, nullable=True),
        ...
        sa.Column('expiry_reason', sa.String(100), nullable=True),
    )
...
class Task(Base):
    """
    Task model for tracking analysis jobs.

    Stores task configuration, status, progress, and results.
    Links to User for ownership and CreditTransaction for billing.
    """
    ...
    parameters = Column(JSON, nullable=True)  # Agent-specific params
    ...
    agents_completed = Column(JSON, nullable=True)  # List of completed agent IDs

    # File Information
    files_metadata = Column(JSON, nullable=False)  # {primary: {filename, size, type}}
    s3_input_keys = Column(JSON, nullable=True)    # List of S3 keys for inputs
    s3_output_keys = Column(JSON, nullable=True)   # List of S3 keys for outputs

    # Results
    result_summary = Column(JSON, nullable=True)   # Summary for quick display
    result_full = Column(LONGTEXT, nullable=True)  # Full results (JSON string)

    # Error Information
    error_code = Column(String(50), nullable=True)
    error_message = Column(Text, nullable=True)
    failed_agent = Column(String(100), nullable=True)
    partial_results = Column(JSON, nullable=True)  # Results from completed agents

    # Timestamps
    created_at = Column(DateTime, server_default=func.now(), nullable=False)
    upload_started_at = Column(DateTime, nullable=True)
    files_verified_at = Column(DateTime, nullable=True)
    ...
    expiry_reason = Column(String(100), nullable=True)
    ...
    def can_process(self) -> bool:
        ...
        return self.status in [
            ...
            TaskStatus.UPLOAD_FAILED.value  # Allow retry after upload failure
        ]
...
```python
# db/schemas.py - Add task schemas
...
class FileMetadata(BaseModel):
    """File metadata for task creation."""
    filename: str
...
class ProgressDetail(BaseModel):
    ...
    agents_status: Dict[str, str]  # agent_id -> status
...
| result_full     | 90 days                  | Clear field, keep summary |

### Cleanup Queries

```sql
-- Mark old tasks for cleanup
UPDATE tasks
SET result_full = NULL
WHERE completed_at < NOW() - INTERVAL 90 DAY
AND result_full IS NOT NULL;
