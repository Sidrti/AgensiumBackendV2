...
def transform_my_tool_response(
    ...
    # 3. Generate AI Analysis Summary
    ...
    try:
        summary_ai = AnalysisSummaryAI()
        analysis_summary = summary_ai.generate_summary(complete_analysis_text)
    except Exception as e:
        analysis_summary = {"status": "error", "summary": str(e)}

    # 4. Generate Routing Recommendations
    # Pass executive_summary and analysis_summary (NOT raw agent_results)
    routing_decisions = []
    try:
        ...
        routing_decisions = routing_ai.get_routing_decisions(
            current_tool="my-tool-id",
            agent_results=agent_results,  # Used only for success_rate calculation
            executive_summary=executive_summary,  # Structured KPIs
            analysis_summary=analysis_summary,    # AI-generated summary text
            primary_filename="data.csv",
            baseline_filename=None,
            current_parameters=None
        )
    except Exception as e:
        print(f"Warning: Routing AI failed: {e}")
...
## 5. Routing Decision AI (`ai/routing_decision_ai.py`)

The `RoutingDecisionAI` class provides intelligent next-tool recommendations based on analysis results.

### Key Features

1.  **Dynamic Tool Loading:** Tools are loaded dynamically from `backend/tools/*.json` files at initialization. No need to hardcode tool definitions.
2.  **Uses Transformer Outputs:** Routing decisions are based on `executive_summary` and `analysis_summary` from the transformer (NOT raw agent_results).
3.  **Success Rate Calculation:** Still uses `agent_results` to calculate success rate for reliability assessment.
...
routing_decisions = routing_ai.get_routing_decisions(
    current_tool="my-tool-id",
    agent_results=agent_results,       # For success_rate only
    executive_summary=executive_summary, # Structured KPIs from transformer
    analysis_summary=analysis_summary,   # AI summary dict from transformer
    primary_filename="data.csv",
    baseline_filename=None,
    current_parameters=None
)
...
| -------------------- | ------------ | ------------------------------------------------------------------ |
| `current_tool`       | `str`        | Current tool ID (e.g., "profile-my-data")                          |
| `agent_results`      | `Dict`       | Raw agent outputs (used for success_rate calculation only)         |
| `executive_summary`  | `List[Dict]` | Structured KPIs with summary_id, title, value, status, description |
| `analysis_summary`   | `Dict`       | AI-generated summary with status, summary text, execution_time_ms  |
| `primary_filename`   | `str`        | Primary data filename                                              |
| `baseline_filename`  | `str`        | Optional baseline filename                                         |
| `current_parameters` | `Dict`       | Current tool parameters                                            |

### Why This Architecture?

- **Scalability:** No need to modify routing logic when adding new agents - it uses standardized transformer outputs.
- **Maintainability:** Tool definitions are loaded from JSON, not hardcoded.
- **Separation of Concerns:** Routing AI doesn't need to understand agent-specific output structures.
